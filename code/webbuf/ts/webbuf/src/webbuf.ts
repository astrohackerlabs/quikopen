import { encodeUnsignedArray, decodeUnsignedArray } from "./unsigned-arrays.js";
import {
  base64UrlToBase64,
  base64ToBase64Url,
  requireBoolean,
...
function verifyOffset(offset: number, ext: number, length: number): void {
...
export class WebBuf implements Iterable<number> {
  ...
  static alloc(size: number, fill = 0): WebBuf {
  ...
  /** Copy a native view's selected raw bytes, without element conversion. */
  ...
  /** Serialize Uint16Array numeric values in BE order into independent bytes. */
  static fromUint16ArrayBE(values: Uint16Array): WebBuf {
    return WebBuf.view(encodeUnsignedArray(values, 2, false));
  }

  /** Decode selected BE bytes into an independent Uint16Array; reject incomplete elements. */
  toUint16ArrayBE(): Uint16Array<ArrayBuffer> {
    return decodeUnsignedArray(this.bytes, 2, false);
  }

  /** Serialize Uint16Array numeric values in LE order into independent bytes. */
  static fromUint16ArrayLE(values: Uint16Array): WebBuf {
    return WebBuf.view(encodeUnsignedArray(values, 2, true));
  }

  /** Decode selected LE bytes into an independent Uint16Array; reject incomplete elements. */
  toUint16ArrayLE(): Uint16Array<ArrayBuffer> {
    return decodeUnsignedArray(this.bytes, 2, true);
  }

  /** Serialize Uint32Array numeric values in BE order into independent bytes. */
  static fromUint32ArrayBE(values: Uint32Array): WebBuf {
    return WebBuf.view(encodeUnsignedArray(values, 4, false));
  }

  /** Decode selected BE bytes into an independent Uint32Array; reject incomplete elements. */
  toUint32ArrayBE(): Uint32Array<ArrayBuffer> {
    return decodeUnsignedArray(this.bytes, 4, false);
  }

  /** Serialize Uint32Array numeric values in LE order into independent bytes. */
  static fromUint32ArrayLE(values: Uint32Array): WebBuf {
    return WebBuf.view(encodeUnsignedArray(values, 4, true));
  }

  /** Decode selected LE bytes into an independent Uint32Array; reject incomplete elements. */
  toUint32ArrayLE(): Uint32Array<ArrayBuffer> {
    return decodeUnsignedArray(this.bytes, 4, true);
  }

  /** Serialize BigUint64Array numeric values in BE order into independent bytes. */
  static fromBigUint64ArrayBE(values: BigUint64Array): WebBuf {
    return WebBuf.view(encodeUnsignedArray(values, 8, false));
  }

  /** Decode selected BE bytes into an independent BigUint64Array; reject incomplete elements. */
  toBigUint64ArrayBE(): BigUint64Array<ArrayBuffer> {
    return decodeUnsignedArray(this.bytes, 8, false);
  }

  /** Serialize BigUint64Array numeric values in LE order into independent bytes. */
  static fromBigUint64ArrayLE(values: BigUint64Array): WebBuf {
    return WebBuf.view(encodeUnsignedArray(values, 8, true));
  }

  /** Decode selected LE bytes into an independent BigUint64Array; reject incomplete elements. */
  toBigUint64ArrayLE(): BigUint64Array<ArrayBuffer> {
    return decodeUnsignedArray(this.bytes, 8, true);
  }
  ...
  static fromString(
    str: string,
    encoding: "utf8" | "hex" | "base64" | "base64url" = "utf8",
  ): WebBuf {
    if (encoding === "base64url") return WebBuf.fromBase64Url(str);
    if (encoding === "hex") {
  ...
  /** Decode strict Base64url with optional complete padding and whitespace opt-in. */
  static fromBase64Url(text: string, stripWhitespace = false): WebBuf {
    return WebBuf.fromBase64(base64UrlToBase64(text, stripWhitespace));
  }

  /** Decode strict Base64url using the JavaScript Base64 backend. */
  static fromBase64UrlPureJs(text: string, stripWhitespace = false): WebBuf {
    return WebBuf.fromBase64PureJs(base64UrlToBase64(text, stripWhitespace));
  }

  /** Decode strict Base64url using the WASM Base64 backend. */
  static fromBase64UrlWasm(text: string, stripWhitespace = false): WebBuf {
    return WebBuf.fromBase64Wasm(base64UrlToBase64(text, stripWhitespace));
  }

  /** Encode selected bytes as Base64url; trailing padding is off by default. */
  toBase64Url(padding = false): string {
    requireBoolean(padding);
    return base64ToBase64Url(this.toBase64(), padding);
  }

  /** Encode Base64url in bounded JavaScript chunks, including large buffers. */
  toBase64UrlPureJs(padding = false): string {
    requireBoolean(padding);
    const chunks: string[] = [];
    // Multiples of three avoid padding between chunks; bound argument spreading.
    for (let offset = 0; offset < this.length; offset += 12288) {
      chunks.push(this.subarray(offset, offset + 12288).toBase64PureJs());
    }
    return base64ToBase64Url(chunks.join(""), padding);
  }

  /** Encode Base64url with the WASM Base64 backend. */
  toBase64UrlWasm(padding = false): string {
    requireBoolean(padding);
    return base64ToBase64Url(this.toBase64Wasm(), padding);
  }
  ...
  static from(
    ...
    if (typeof mapFn === "string") {
      ...
      if (mapFn === "base64url") return WebBuf.fromBase64Url(source);
      if (mapFn === "utf8") {
  ...
  toString(encoding?: "utf8" | "hex" | "base64" | "base64url"): string {
    if (encoding === "base64url") return this.toBase64Url();
    if (encoding === "hex") {
      return this.toHex();
    }
    if (encoding === "base64") {
  ...
  compare(other: WebBuf): number {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (a === undefined || b === undefined) {
        throw new RangeError("Comparison byte is outside the buffer");
      }
      if (a !== b) {
        return a < b ? -1 : 1;
      }
  ...
  /**
   * Whether every selected byte is zero; empty buffers return true.
