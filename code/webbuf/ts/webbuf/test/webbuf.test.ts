...
describe("WebBuf", () => {
  ...
  describe("compare", () => {
    it("compares unsigned bytes in offset views without changing storage", () => {
      for (const left of [0, 1, 127, 128, 254, 255]) {
        for (const right of [0, 1, 127, 128, 254, 255]) {
          const a = WebBuf.fromArray([255, 0, left, 0, 255]);
          const b = WebBuf.fromArray([1, 0, right, 0, 1]);
          const aView = a.subarray(1, 4);
          const bView = b.subarray(1, 4);
          expect(aView.compare(bView)).toBe(Math.sign(left - right));
          expect(WebBuf.compare(bView, aView)).toBe(Math.sign(right - left));
          expect(a.toArray()).toEqual([255, 0, left, 0, 255]);
          expect(b.toArray()).toEqual([1, 0, right, 0, 1]);
        }
      }
    });
