...
/** Check indexed storage before converting a byte to its integer value. */
function byteToBigInt(byte: number | undefined): bigint {
  if (byte === undefined) {
...
export class U8 extends FixedNum<1> {
  ...
  toBn(): bigint {
    return byteToBigInt(this.buf.buf.bytes[0]);
  }
...
export class U16BE extends FixedNum<2> {
  ...
  toBn(): bigint {
    return (
      (byteToBigInt(this.buf.buf.bytes[0]) << 8n) +
      byteToBigInt(this.buf.buf.bytes[1])
    );
  }
...
export class U16LE extends FixedNum<2> {
  ...
  toBn(): bigint {
    return (
      (byteToBigInt(this.buf.buf.bytes[1]) << 8n) +
      byteToBigInt(this.buf.buf.bytes[0])
    );
  }
...
export class U32BE extends FixedNum<4> {
  ...
  static fromBn(bn: bigint): U32BE {
    ...
    const bytes = new Array<number>(byteLen);
  ...
  toBn(): bigint {
    ...
    for (let i = 0; i < byteLen; i++) {
      result = (result << 8n) + byteToBigInt(this.buf.buf.bytes[i]);
    }
    return result;
  }
