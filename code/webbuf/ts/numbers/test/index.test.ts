...
describe("fixed-width byte conversion", () => {
  it("rejects missing backing bytes for every width and byte order", () => {
    for (const NumberType of [
      ...
      const value = new NumberType(0n);
      // FixedBuf exposes mutable backing storage; preserve the existing
      // TypeError instead of silently interpreting a missing byte as zero.
      value.buf._buf = WebBuf.alloc(value.buf.buf.length - 1);
      expect(() => value.toBn()).toThrow(TypeError);
    }
  });
});
